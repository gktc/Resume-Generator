import { useState } from 'react';
import { Skill, SkillInput } from '../types/profile.types';
import SkillForm from './SkillForm';

interface SkillsManagerProps {
  skills: Skill[];
  onAdd: (data: SkillInput) => Promise<void>;
  onUpdate: (id: string, data: SkillInput) => Promise<void>;
  onDelete: (id: string) => Promise<void>;
}

const SkillsManager = ({ skills, onAdd, onUpdate, onDelete }: SkillsManagerProps) => {
  const [showForm, setShowForm] = useState(false);
  const [editingSkill, setEditingSkill] = useState<Skill | null>(null);

  const handleAdd = () => {
    setEditingSkill(null);
    setShowForm(true);
  };

  const handleEdit = (skill: Skill) => {
    setEditingSkill(skill);
    setShowForm(true);
  };

  const handleDelete = async (id: string) => {
    if (!confirm('Are you sure you want to delete this skill?')) {
      return;
    }
    await onDelete(id);
  };

  const handleSubmit = async (data: SkillInput) => {
    if (editingSkill) {
      await onUpdate(editingSkill.id, data);
    } else {
      await onAdd(data);
    }
    setShowForm(false);
    setEditingSkill(null);
  };

  const handleCancel = () => {
    setShowForm(false);
    setEditingSkill(null);
  };

  const groupedSkills = (skills || []).reduce(
    (acc, skill) => {
      if (!acc[skill.category]) {
        acc[skill.category] = [];
      }
      acc[skill.category].push(skill);
      return acc;
    },
    {} as Record<string, Skill[]>
  );

  const getProficiencyBadge = (proficiency: string) => {
    switch (proficiency) {
      case 'beginner':
        return 'badge-neutral';
      case 'intermediate':
        return 'badge-info';
      case 'advanced':
        return 'badge-success';
      case 'expert':
        return 'badge-warning';
      default:
        return 'badge-neutral';
    }
  };

  const getCategoryIcon = (category: string) => {
    switch (category) {
      case 'technical':
        return (
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              strokeWidth={2}
              d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"
            />
          </svg>
        );
      case 'soft':
        return (
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              strokeWidth={2}
              d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"
            />
          </svg>
        );
      case 'language':
        return (
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              strokeWidth={2}
              d="M3 5h12M9 3v2m1.048 9.5A18.022 18.022 0 016.412 9m6.088 9h7M11 21l5-10 5 10M12.751 5C11.783 10.77 8.07 15.61 3 18.129"
            />
          </svg>
        );
      default:
        return null;
    }
  };

  if (showForm) {
    return <SkillForm skill={editingSkill} onSubmit={handleSubmit} onCancel={handleCancel} />;
  }

  if (!skills || skills.length === 0) {
    return (
      <div className="card">
        <div className="empty-state">
          <svg className="empty-state-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              strokeWidth={2}
              d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"
            />
          </svg>
          <h3 className="section-header">No skills yet</h3>
          <p className="empty-state-text mb-4">Add your first skill to get started</p>
          <button onClick={handleAdd} className="btn-primary">
            Add Skill
          </button>
        </div>
      </div>
    );
  }

  return (
    <div>
      <div className="flex justify-end mb-6">
        <button onClick={handleAdd} className="btn-primary flex items-center gap-2">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
          </svg>
          Add Skill
        </button>
      </div>

      <div className="space-y-6">
        {Object.entries(groupedSkills).map(([category, categorySkills]) => (
          <div key={category} className="card">
            <div className="card-header">
              <div className="flex items-center gap-3">
                <div className="badge-info">{getCategoryIcon(category)}</div>
                <h2 className="section-header capitalize">
                  {category} Skills ({categorySkills.length})
                </h2>
              </div>
            </div>

            <div className="card-body">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {categorySkills.map((skill) => (
                  <div key={skill.id} className="list-item">
                    <div className="flex justify-between items-start mb-2">
                      <h3 className="form-label">{skill.name}</h3>
                      <div className="flex gap-1">
                        <button
                          onClick={() => handleEdit(skill)}
                          className="btn-icon-sm btn-secondary"
                          title="Edit"
                        >
                          <svg
                            className="w-4 h-4"
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                          >
                            <path
                              strokeLinecap="round"
                              strokeLinejoin="round"
                              strokeWidth={2}
                              d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"
                            />
                          </svg>
                        </button>
                        <button
                          onClick={() => handleDelete(skill.id)}
                          className="btn-icon-sm btn-danger"
                          title="Delete"
                        >
                          <svg
                            className="w-4 h-4"
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                          >
                            <path
                              strokeLinecap="round"
                              strokeLinejoin="round"
                              strokeWidth={2}
                              d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"
                            />
                          </svg>
                        </button>
                      </div>
                    </div>
                    <div className="space-y-2">
                      <span className={`badge ${getProficiencyBadge(skill.proficiency)}`}>
                        {skill.proficiency}
                      </span>
                      {skill.yearsOfExperience !== null && (
                        <p className="form-helper-text">
                          {skill.yearsOfExperience}{' '}
                          {skill.yearsOfExperience === 1 ? 'year' : 'years'} of experience
                        </p>
                      )}
                    </div>
                  </div>
                ))}
              </div>
            </div>
          </div>
        ))}
      </div>
    </div>
  );
};

export default SkillsManager;
